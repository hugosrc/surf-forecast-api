...
describe('Beaches functional tests', () => {
  ...
  describe('When deleting a beach', () => {
    beforeEach(async () => {
      await Beach.deleteMany({});
      await User.deleteMany({});
    ...
    it('should delete a beach with success', async () => {
      const response = await global.testRequest
        .delete(`/beaches/${createdBeachId}`)
        .set({ authorization: 'Bearer ' + token })
        .send();
    ...
    it('should return forbidden error when user tries to delete another users beach', async () => {
      const newUser: User = {
        name: 'Fake',
        email: 'fake@mail.com',
        password: '1234',
